...
@inject
def list_groups(service: GroupService = Depends(Provide[Container.group_service])):
    return [_to_response(item) for item in service.list_groups()]


@router.get('/{group_id}', response_model=GroupResponse)
@inject
def get_group(group_id: str, service: GroupService = Depends(Provide[Container.group_service])):
    try:
...
@inject
def create_group(body: GroupCreateRequest, service: GroupService = Depends(Provide[Container.group_service])):
    try:
    ...
    except GroupMemberMustBeContact:
        raise HTTPException(status_code=400, detail='All members must be contacts of the creator')


@router.delete('/{group_id}')
@inject
def delete_group(group_id: str, body: GroupDeleteRequest, service: GroupService = Depends(Provide[Container.group_service])):
    try:
        service.delete_group(group_id, body.actor_id)
...
@inject
def add_user_to_group(group_id: str, body: AddUserToGroupRequest, service: GroupService = Depends(Provide[Container.group_service])):
    try:
...
@inject
def delete_user_from_group(group_id: str, user_id: str, actor_id: str, service: GroupService = Depends(Provide[Container.group_service])):
    try:
...
@inject
def update_admin(group_id: str, body: UpdateAdminRequest, service: GroupService = Depends(Provide[Container.group_service])):
    try:
...
@inject
def exit_group(group_id: str, user_id: str, service: GroupService = Depends(Provide[Container.group_service])):
    try:
