...
class GroupInMemoryRepository(GroupRepository):
    ...
    def delete(self, group_id: str) -> None:
        self._store = [row for row in self._store if row['id'] != group_id]
    ...
    def _row_to_entity(row: dict) -> GroupEntity:
        return GroupEntity(
            id=row['id'], name=row['name'], description=row['description'], created_by=row['created_by'],
            created_at=row['created_at'], members=list(row['members']), admins=list(row['admins']), invitation_link=row.get('invitation_link')
        )
